...
void Audio::init () {
  ...
  Audio::device = alcOpenDevice(DeviceName);
  if (Audio::device) {
    std::cout << "Created audio device: " << DeviceName << std::endl;
  }
  std::cout << "Device: " << alcGetString(Audio::device, ALC_DEVICE_SPECIFIER) << std::endl;
  alGetError();
